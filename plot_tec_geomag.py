...
con = ax.pcolormesh(cdlon, cdlat, vtec['tec'].values[itime], vmin=vmin, vmax=vmax, transform=data_crs, cmap = cmap)
...
def update(frame):
    ...
    new_lon = frame % 360
    # Re-project for new longitude
    ax.projection = ccrs.Orthographic(central_longitude=new_lon, central_latitude=90)
    ax.coastlines()
